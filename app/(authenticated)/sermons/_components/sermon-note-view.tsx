...
export function SermonNoteView({ sermonNotes }: SermonNoteViewProps) {
  ...
  const filteredAndSortedSermonNotes = sermonNotes
    ?.filter((sermonNote) => {
      const searchTerm = searchQuery.toLowerCase();
      return (
        sermonNote.title.toLowerCase().includes(searchTerm) ||
        (sermonNote.markdown?.toLowerCase().includes(searchTerm) ?? false) ||
        (sermonNote.ocrText?.toLowerCase().includes(searchTerm) ?? false)
      );
    })
    .sort((a, b) => {
  ...
  return (
    <div>
      ...
      <SermonNoteGrid sermonNotes={filteredAndSortedSermonNotes ?? []} />
    </div>
  );
}
